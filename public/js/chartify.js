...
function modEvoChart2(data){
  ...
  //Add file names to series
  result1.series.push(data.Results[6].Name);
  result1.series.push(data.Results[7].Name);

  //loop over first file to get lables and first info set
  var lines = splitByLine(data.Results[6].Data);
  var s1 = []
  for (var i = 1; i < lines.length; i++) {
    var parts = lines[i].split(" ");
    result1.labels.push(parts[0]);
  ...
  //loop over second file to get second info set
  var lines = splitByLine(data.Results[7].Data);
  var s2 = []
  for (var i = 1; i < lines.length; i++) {
    var parts = lines[i].split(" ");
    s2.push(parts[1]);
