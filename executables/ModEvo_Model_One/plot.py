...
with open("meanTraitOneValues_GeneralModel_1.txt") as f:
    next(f)
    for line in f:
...
with open("meanTraitTwoValues_GeneralModel_1.txt") as f:
    next(f)
    for line in f:
        stuff = line.rstrip().split(" ")
        value = float(stuff[1])
...
file.write("plot 'contourBase.dat' with image, 'contourLines.dat' w l lt -1 lw 1, \"< paste meanTraitOneValues_GeneralModel_1.txt meanTraitTwoValues_GeneralModel_1.txt\" using 2:4 with lines\n")
file.close();
